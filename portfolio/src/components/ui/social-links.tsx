...
export function SocialLinks({ socials, className, ...props }: SocialLinksProps) {
  ...
  return (
    <div
      className={cn("flex items-center justify-center gap-0 ems-center justify-center gap-4 fixed bottom-8 left-0 right-0", className)}
      {...props}
    >
        ...
        >
          ...
          <AnimatePresence>
            {hoveredSocial === social.name && (
              <motion.div
                className="absolute left-0 right-0 flex h-full w-full items-center justify-center"
                animate={animation}
              >
                <motion.img
                  key={social.name}
